...
export default function AdSlot({ position, className = "" }: AdSlotProps) {
  ...
  return (
    <div className={`ad-container rounded-xl overflow-hidden ${className}`} ref={adRef}>
      ...
      <div className="flex flex-col items-center justify-center w-full h-full min-h-[90px] bg-gradient-to-r from-amber-50 to-emerald-50 border border-dashed border-gray-200 rounded-xl">
        ...
        <p className="text-[10px] text-gray-400 uppercase tracking-wider">{config.label}</p>
      </div>
    </div>
  )
}
